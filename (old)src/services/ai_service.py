...
usa AI_MODEL/AI_API_KEY globais de settings como fallback para o modo standalone.
"""
import uuid
import time
...
class AIService:
    ...
    def _build_tools(self, agent_id: str):
        ...
        api_tool = (
            ApiTool(url=str(api_cfg.url), token=api_cfg.token, query_param=api_cfg.query_param)
            if api_cfg else None
        )
        webhook_tool = (
            WebhookTool(url=str(webhook_cfg.url), token=webhook_cfg.token)
            if webhook_cfg else None
        )
        sql_tool = (
            SqlTool(
