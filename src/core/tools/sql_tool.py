...
Recebe SQL gerado pelo LLM; não traduz linguagem natural.
...
def validate_connection_string(conn_str: str) -> str:
    """
    Camadas 1 e 2: valida formato e dialeto.
    Retorna o conn_str se válido; levanta ValueError caso contrário.
    """
    try:
    ...
    if dialect not in allowed:
        raise ValueError(
            f"Dialeto '{dialect}' não permitido. Permitidos: {', '.join(allowed)}."
        )
...
def _validate_sql(sql: str) -> str:
    """
    Camada 3: garante que a query é um único SELECT.
    Levanta ValueError se detectar DDL, DML ou múltiplos statements.
    """
    sql = sql.strip().rstrip(";")

    # Rejeita múltiplos statements
    if ";" in sql:
...
def _audit(agent_id: str, sql: str, rows: int, duration_ms: int, error: str | None) -> None:
    """Camada 5: registra a execução em JSONL por agente."""
    try:
        ...
        pass  # falha no audit não deve interromper a resposta
...
class SqlTool:
    ...
    def _connect_string(self) -> str:
        """Descriptografa e valida a connection string."""
        plaintext = decrypt_secret(self._conn_str_enc)
        return validate_connection_string(plaintext)

    def _get_schema(self) -> str:
        """Introspecta o banco e retorna descrição das tabelas para o LLM."""
        ...
        try:
            ...
            parts = []
            for table in tables[:20]:  # limite para não estourar context window
                try:
    ...
    def get_tool_definition(self) -> dict:
        """Retorna a definição de tool com o schema injetado na descrição."""
    ...
    def execute(self, sql: str) -> str:
        """
        Camadas 3, 4 e 5: valida, executa com timeout e retorna resultado formatado.
        Levanta ValueError (SQL inválido), TimeoutError (timeout), RuntimeError (erro DB).
        """
        try:
        ...
        engine = create_engine(
            conn_str,
            pool_pre_ping=True,
            connect_args=self._timeout_args(conn_str),
        )
        ...
        try:
            with engine.connect() as conn:
                # Camada 4: timeout via statement_timeout (PostgreSQL) ou execution_options
                result = conn.execute(
                    text(safe_sql),
                    execution_options={"timeout": settings.SQL_QUERY_TIMEOUT},
                )
    ...
    def _timeout_args(conn_str: str) -> dict:
        """Retorna connect_args de timeout adequados ao dialeto."""
